...
import NadeLineupViewer3D from "~/components/nades/NadeLineupViewer3D.vue";
...
import NadeMissPatternPanel from "~/components/nades/NadeMissPatternPanel.vue";
...
import NadeSightlinePanel from "~/components/nades/NadeSightlinePanel.vue";
...
import {
  clearNadeVoteMutation,
  favoriteNadeLineupMutation,
  nadeLineupQuery,
  nadeMetaLineupsQuery,
  setNadeVoteMutation,
  unfavoriteNadeLineupMutation,
...
const lineup = ref<NadeLineup | null>(null);
...
async function fetchLineup() {
  ...
  try {
    const { data } = await getGraphqlClient().query({
      query: nadeLineupQuery,
      variables: { id: lineupId.value },
      fetchPolicy: "network-only",
    });
    lineup.value = (data as any)?.nade_lineups_by_pk ?? null;
  } catch (error) {
...
async function fetchMetaThrowers() {
  ...
  try {
    const { data } = await getGraphqlClient().query({
      ...
      variables: {
        where: {
          ...
          nade_type: { _eq: current.nade_type },
        },
    ...
    console.error("[nades] meta lookup error:", error);
    metaThrowers.value = null;
  }
}
...
async function vote(value: 1 | -1) {
  ...
  try {
    ...
    const where = {
      nade_lineup_id: { _eq: current.id },
      steam_id: { _eq: mySteamId.value },
    };
    if (clearing) {
      await client.mutate({
        mutation: clearNadeVoteMutation,
        variables: { where },
      });
    } else {
      await client.mutate({
        mutation: setNadeVoteMutation,
        variables: {
          where,
          object: { nade_lineup_id: current.id, vote: value },
        },
      });
    }
    await fetchLineup();
    ...
    toast({
      title: t("pages.nades.detail.vote_failed"),
...
async function toggleFavorite() {
  ...
  try {
    ...
    if (current.is_favorited) {
      await client.mutate({
        mutation: unfavoriteNadeLineupMutation,
        variables: {
          where: {
            nade_lineup_id: { _eq: current.id },
            steam_id: { _eq: mySteamId.value },
      ...
      await client.mutate({
        mutation: favoriteNadeLineupMutation,
        variables: {
          object: { nade_lineup_id: current.id },
        },
      });
    }
    await fetchLineup();
    ...
    toast({
      title: t("pages.nades.detail.favorite_failed"),
      description: error?.message,
      variant: "destructive",
    });
  }
...
<template>
  ...
  <PageTransition v-else-if="!lineup">
    <Empty>
      <EmptyTitle>{{ $t("pages.nades.detail.not_found") }}</EmptyTitle>
      <EmptyDescription>
        {{ $t("pages.nades.detail.not_found_description") }}
      </EmptyDescription>
    </Empty>
  </PageTransition>
      ...
      <TacticalPageHeader>
        ...
        <template #actions>
          ...
          <Button class="tac-amber-cta" @click="practiceOpen = true">
            ...
            {{ $t("pages.nades.detail.practice_this") }}
          </Button>
        </template>
      </TacticalPageHeader>
    </PageTransition>
    ...
    <PageTransition :delay="60" class="mt-4">
      <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div class="flex flex-col gap-2">
          <NadeLineupViewer3D v-if="hasMesh" :lineup="lineup" />
          <template v-else>
            <NadeRadarBoard
              :map-name="mapName"
              :lineups="[lineup]"
            ...
            <p class="flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              {{ $t("pages.nades.detail.no_mesh") }}
            </p>
          </template>
        </div>

        <div class="flex flex-col gap-3">
          <NadeConfidenceNote :lineup="lineup" />
          ...
          <div class="flex flex-wrap items-center gap-1.5">
            ...
            <Badge
              ...
              class="font-mono text-[0.62rem] uppercase"
            >
              {{ $t(`pages.nades.visibility.${lineup.visibility}`) }}
            </Badge>
            <NadeDifficultyChip :difficulty="lineup.difficulty" />
            <span
              v-if="metaThrowers"
          ...
          >
            <div>
              <dt class="text-muted-foreground">
                {{ $t("pages.nades.detail.flight_time") }}
              </dt>
              <dd class="font-mono tabular-nums">
                {{
                  flightSeconds
                    ? $t("pages.nades.card.flight_time", {
            ...
            <div>
              <dt class="text-muted-foreground">
                {{ $t("pages.nades.detail.view_angles") }}
              </dt>
              <dd class="font-mono tabular-nums">
            ...
            <div>
              <dt class="text-muted-foreground">
                {{ $t("pages.nades.detail.origin_source") }}
              </dt>
              <dd class="font-mono uppercase">
                {{ $t(`pages.nades.origin_sources.${lineup.origin_source}`) }}
              </dd>
            </div>
            <div>
              <dt class="text-muted-foreground">
                {{ $t("pages.nades.detail.verified") }}
              </dt>
              <dd class="font-mono uppercase">
            ...
            >
              <dt class="text-muted-foreground">
                {{ $t("pages.nades.detail.aim_delta") }}
              </dt>
              <dd class="font-mono tabular-nums">
          ...
          <NadeProgressPanel :progress="lineup.progress" />

          <NadeMissPatternPanel :lineup-id="lineup.id" />
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <NadeCollectionPicker :lineup-id="lineup.id" />
      ...
      >
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            ...
            >
              {{ $t("pages.nades.sightline.eyebrow") }}
            </div>
            <h2 class="text-sm font-semibold">
              {{ $t("pages.nades.sightline.title") }}
            </h2>
            <p class="mt-0.5 text-xs text-muted-foreground">
              {{ $t("pages.nades.sightline.description") }}
            </p>
          </div>
          <Button
            variant="outline"
          ...
          >
            ...
            {{
              sightlineOpen
                ? $t("pages.nades.sightline.hide")
                : $t("pages.nades.sightline.open")
            }}
          </Button>
        </div>

        <div v-if="sightlineOpen" class="mt-3">
          <NadeSightlinePanel :lineup="lineup" />
        </div>
      </div>
    </PageTransition>
