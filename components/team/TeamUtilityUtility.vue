...
const report = ref<UtilityTeamUtilityView | null>(null);
...
async function load() {
  ...
  try {
    const { data } = await apolloClient.query({
      query: utilityTeamUtilityReportQuery,
      variables: {
    ...
    const view = readUtilityTeamUtility(
      (data as any)?.utilityTeamUtilityReport as UtilityTeamUtilityOutput | undefined,
    );
    report.value = view;
    lineupsById.value = {};
    ...
    if (gen === loadGen) {
      console.error("[utility] team utility report error:", error);
      report.value = null;
      lineupsById.value = {};
    }
  } finally {
  ...
  <div class="flex flex-col gap-4">
    <div class="flex flex-col gap-1">
      <span :class="tacticalSectionLabelClasses">
        ...
        {{ $t("team.utility_utility.title") }}
      </span>
      <span :class="tacticalSectionDescriptionClasses">
        {{ $t("team.utility_utility.description") }}
      </span>
    </div>
    ...
    >
      ...
      <div class="min-w-0 text-sm">
        <div class="font-medium">
          {{ $t("team.utility_utility.not_analysed") }}
        </div>
        <p
          class="mt-0.5 whitespace-pre-wrap break-words text-xs leading-relaxed text-muted-foreground"
        >
          {{
            report.message || $t("team.utility_utility.not_analysed_description")
          }}
        </p>
      </div>
    </div>
    ...
    <Empty v-else-if="!report">
      <EmptyTitle>{{ $t("team.utility_utility.failed") }}</EmptyTitle>
      <EmptyDescription>
        {{ $t("team.utility_utility.failed_description") }}
      </EmptyDescription>
    </Empty>

    <Empty v-else-if="!sortedRows.length">
      <EmptyTitle>{{ $t("team.utility_utility.empty") }}</EmptyTitle>
      <EmptyDescription>
        {{ $t("team.utility_utility.empty_description") }}
      </EmptyDescription>
    </Empty>
    ...
    <template v-else>
          ...
          <TableHeader class="[&_th]:h-10 bg-muted/20">
            <TableRow>
              ...
              <SortableTableHead
                ...
                @sort="toggle"
              >
                {{ $t("team.utility_utility.players") }}
              </SortableTableHead>
              <SortableTableHead
                sort-key="thrown"
                :active-key="sortKey"
              ...
              <SortableTableHead
                ...
                @sort="toggle"
              >
                {{ $t("team.utility_utility.land_rate") }}
              </SortableTableHead>
              <TableHead />
            </TableRow>
          </TableHeader>
            ...
            <TableRow v-for="row of sortedRows" :key="row.lineupId">
              ...
              <TableCell class="text-right tabular-nums">
                {{
                  row.landRate === null
                    ? $t("common.na")
                    : $t("pages.utility.progress.hit_rate", {
              ...
              <TableCell class="text-right">
                ...
                >
                  {{ $t(`pages.utility.types.${row.lineup.utility_type}`) }}
                  ·
                  {{ $t(`pages.utility.sides.${row.lineup.side}`) }}
                </span>
              </TableCell>
            </TableRow>
          </TableBody>
      ...
      <p class="text-xs leading-relaxed text-muted-foreground">
        {{ $t("team.utility_utility.attribution_note") }}
      </p>
    </template>
  </div>
</template>
