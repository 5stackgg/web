...
const key = computed(() => nadeDifficultyKey(token.value));

const measured = computed(() => nadeDifficultyMeasured(token.value));

const label = computed(() => {
  if (key.value) {
    return t(`pages.nades.difficulty.levels.${key.value}`);
  }
  return humanizeNadeToken(token.value);
});

const note = computed(() => {
  if (key.value) {
    return t(`pages.nades.difficulty.notes.${key.value}`);
  }
  return t("pages.nades.difficulty.notes.unknown");
});
</script>
