...
import NadeImportPanel from "~/components/nades/NadeImportPanel.vue";
</script>
...
<template>
  <SettingsPage>
    <PageTransition :delay="0">
      <SettingsSection
        id="library"
        :title="$t('pages.settings.application.nades.library_section')"
        :description="
          $t('pages.settings.application.nades.library_description')
        "
        clickable-header
        @header-click="save('public.nade_library_enabled', !libraryEnabled)"
      >
        <template #action>
          <Switch
            :model-value="libraryEnabled"
            @update:model-value="
              (value) => save('public.nade_library_enabled', value)
            "
          />
        </template>
        ...
        <div v-if="libraryEnabled" class="space-y-2">
          ...
          >
            {{ $t("pages.settings.application.nades.lineup_daily_limit") }}
          </label>
          <p class="text-sm text-muted-foreground">
            {{
              $t(
                "pages.settings.application.nades.lineup_daily_limit_description",
              )
            }}
          </p>
          <Input
            :model-value="drafts['public.nade_lineup_daily_limit']"
            type="number"
            min="0"
            :placeholder="$t('pages.settings.application.nades.server_default')"
            @update:model-value="
              (value) => onNumberInput('public.nade_lineup_daily_limit', value)
            "
          />
          <p
            v-if="invalid['public.nade_lineup_daily_limit']"
            class="text-[0.8rem] font-medium text-destructive"
          >
            {{ $t("pages.settings.application.nades.invalid_number") }}
          </p>
        </div>
      </SettingsSection>
    </PageTransition>
    ...
    <PageTransition :delay="60">
      <SettingsSection
        id="practice"
        :title="$t('pages.settings.application.nades.practice_section')"
        :description="
          $t('pages.settings.application.nades.practice_description')
        "
        clickable-header
        @header-click="save('public.nade_practice_enabled', !practiceEnabled)"
      >
        <template #action>
          <Switch
            :model-value="practiceEnabled"
            @update:model-value="
              (value) => save('public.nade_practice_enabled', value)
            "
          />
        </template>
        ...
        <template v-if="practiceEnabled">
          ...
          >
            <label
              class="font-mono text-[0.7rem] font-medium uppercase tracking-[0.18em] text-muted-foreground"
            >
              {{ $t(`pages.settings.application.nades.${field.label}`) }}
            </label>
            <p class="text-sm text-muted-foreground">
              {{
                $t(`pages.settings.application.nades.${field.label}_description`)
              }}
            </p>
            <Input
              ...
              :placeholder="
                field.fallbackLabel
                  ? $t(
                      `pages.settings.application.nades.${field.fallbackLabel}`,
                    )
                  : $t('pages.settings.application.nades.server_default')
              "
    ...
    <PageTransition :delay="120">
      <SettingsSection
        id="import"
        :title="$t('pages.settings.application.nades.import_section')"
        :description="$t('pages.settings.application.nades.import_description')"
        clickable-header
        @header-click="save('public.nade_import_enabled', !importEnabled)"
      >
        <template #action>
          <Switch
            :model-value="importEnabled"
            @update:model-value="
              (value) => save('public.nade_import_enabled', value)
            "
          />
        </template>

        <NadeImportPanel v-if="importEnabled" />
      </SettingsSection>
    </PageTransition>
...
/**
 * Every numeric knob the nade feature reads. An empty field is not zero — it
...
const NUMBER_SETTINGS = [
  {
    name: "public.nade_lineup_daily_limit",
    label: "lineup_daily_limit",
    fallbackLabel: null,
  },
  {
    name: "public.nade_practice_idle_minutes",
    label: "practice_idle_minutes",
    fallbackLabel: null,
  },
  {
    name: "public.nade_practice_daily_limit",
    label: "practice_daily_limit",
    fallbackLabel: null,
  },
  {
    name: "public.nade_practice_reserved_servers",
    label: "practice_reserved_servers",
    fallbackLabel: null,
  },
  {
    name: "public.nade_success_radius",
...
const PRACTICE_FIELDS = NUMBER_SETTINGS.filter(
  (field) => field.name !== "public.nade_lineup_daily_limit",
);
...
export default {
  ...
  computed: {
    ...
    libraryEnabled(): boolean {
      return this.isEnabled("public.nade_library_enabled");
    },
    practiceEnabled(): boolean {
      return this.isEnabled("public.nade_practice_enabled");
    },
    importEnabled(): boolean {
      return this.isEnabled("public.nade_import_enabled");
    },
    changed(): string[] {
  ...
  methods: {
    ...
    async save(name: string, value: boolean) {
      ...
      toast({ title: this.$t("pages.settings.application.nades.updated") });
    },
    async updateSettings() {
      ...
      if (Object.values(this.invalid).some(Boolean)) {
        toast({
          title: this.$t("pages.settings.application.nades.invalid_number"),
          variant: "destructive",
        });
        return;
      }
