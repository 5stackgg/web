...
const props = withDefaults(
  defineProps<{
    ...
    pickZ?: number;
    markers?: NadeBoardMarker[];
    segments?: NadeBoardSegment[];
...
const markers = computed<Marker[]>(() => {
  ...
  for (const lineup of props.lineups) {
    const origin = projectCalibrated(nadeOrigin(lineup));
    if (!origin) {
      continue;
    }
    const land = nadeLanding(lineup);
    ...
    out.push({
      id: lineup.id,
      color: NADE_TYPE_COLORS[lineup.nade_type] ?? "#ffffff",
      origin,
      landing,
      path,
    });
