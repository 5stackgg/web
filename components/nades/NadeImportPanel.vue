...
import {
  importNadeLineupsMutation,
  purgeNadeLineupSourceMutation,
} from "~/graphql/nadesGraphql";
import {
  NADE_IMPORT_MAX_ENTRIES,
  readNadeImportOutput,
  readNadePurgeOutput,
} from "~/types/nade";
import type {
  NadeImportOutput,
...
// The action takes any source in `e_nade_sources`, and offering that choice from
// here would put "delete everything the plugin recorded" one click from a
// payload box.
const PURGE_SOURCE: NadeOriginSource = "import";

const payloadText = ref("");
const dryRun = ref<NadeImportView | null>(null);
const applied = ref<NadeImportView | null>(null);
const confirmPurge = ref(false);
const purgePreview = ref<NadePurgeView | null>(null);
...
const tooManyEntries = computed(
  () => (entryCount.value ?? 0) > NADE_IMPORT_MAX_ENTRIES,
);
...
async function run(isDryRun: boolean) {
  ...
  try {
    const { data } = await getGraphqlClient().mutate({
      mutation: importNadeLineupsMutation,
      variables: {
        payload,
        dry_run: isDryRun,
      },
    });
    const view = readNadeImportOutput(
      (data as any)?.importNadeLineups as NadeImportOutput | undefined,
    );
    if (isDryRun) {
      dryRun.value = view;
      applied.value = null;
    ...
    applied.value = view;
    toast({
      title: t("pages.settings.application.nades.import.done", {
        count: view.imported,
      }),
    });
  } catch (error: any) {
    toast({
      title: isDryRun
        ? t("pages.settings.application.nades.import.dry_run_failed")
        : t("pages.settings.application.nades.import.failed"),
...
async function runPurge(isDryRun: boolean) {
  const { data } = await getGraphqlClient().mutate({
    mutation: purgeNadeLineupSourceMutation,
    variables: {
      origin_source: PURGE_SOURCE,
      dry_run: isDryRun,
    },
  });
  return readNadePurgeOutput(
    (data as any)?.purgeNadeLineupSource as NadePurgeOutput | undefined,
  );
}
...
async function previewPurge() {
  try {
    ...
    toast({
      title: t("pages.settings.application.nades.import.purge_preview_failed"),
      description: error?.message,
      variant: "destructive",
    });
  }
...
async function purge() {
  try {
    ...
    applied.value = null;
    toast({
      title: t("pages.settings.application.nades.import.purged", {
        count: result.lineups,
      }),
    });
  } catch (error: any) {
    toast({
      title: t("pages.settings.application.nades.import.purge_failed"),
      description: error?.message,
      variant: "destructive",
    });
  }
...
const results = computed(() => {
  ...
  return [
    {
      key: "total",
      label: t("pages.settings.application.nades.import.total"),
      value: view.total,
      tone: "",
    },
    {
      key: "imported",
      label: t("pages.settings.application.nades.import.imported"),
      value: view.imported,
      tone: "",
    },
    {
      key: "updated",
      label: t("pages.settings.application.nades.import.updated"),
      value: view.updated,
      tone: "",
    },
    {
      key: "failed",
      label: t("pages.settings.application.nades.import.failed_count"),
      value: view.failed,
...
<template>
  <div class="space-y-3">
    <div class="space-y-1">
      ...
      >
        {{ $t("pages.settings.application.nades.import.payload") }}
      </label>
      <p class="text-sm text-muted-foreground">
        {{ $t("pages.settings.application.nades.import.payload_description") }}
      </p>
    </div>
    ...
    <Textarea
      ...
      :placeholder="
        $t('pages.settings.application.nades.import.payload_placeholder')
      "
    />
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <Button variant="outline" size="sm" @click="pickFile()">
        ...
        {{ $t("pages.settings.application.nades.import.choose_file") }}
      </Button>
      ...
      >
        {{
          $t("pages.settings.application.nades.import.entries", {
            count: entryCount,
          })
        }}
      </span>
      ...
      >
        ...
        {{ $t("pages.settings.application.nades.import.dry_run") }}
      </Button>
      ...
      <Button
        ...
        @click="run(false)"
      >
        {{ $t("pages.settings.application.nades.import.apply") }}
      </Button>
    </div>
    ...
    >
      {{ $t("pages.settings.application.nades.import.invalid_json") }}
    </p>
    ...
    >
      {{
        $t("pages.settings.application.nades.import.too_many_entries", {
          max: NADE_IMPORT_MAX_ENTRIES,
        })
      }}
    </p>

    <p v-if="!dryRun && !applied" class="text-xs text-muted-foreground">
      {{ $t("pages.settings.application.nades.import.dry_run_first") }}
    </p>
    ...
    <div v-if="shownResult" class="space-y-2">
      ...
      >
        {{
          shownResult.dryRun
            ? $t("pages.settings.application.nades.import.dry_run_label")
            : $t("pages.settings.application.nades.import.applied_label")
        }}
      </div>
      ...
      <p v-if="shownResult.empty" class="text-xs text-muted-foreground">
        {{ $t("pages.settings.application.nades.import.nothing_in_payload") }}
      </p>
      ...
      <div v-if="shownResult.errors.length" class="space-y-1">
        ...
        >
          ...
          {{
            $t("pages.settings.application.nades.import.errors", {
        ...
        >
          ...
          >
            <span class="shrink-0 font-semibold tabular-nums">
              {{
                $t("pages.settings.application.nades.import.error_row", {
                  index: error.index,
                })
              }}
            </span>
        ...
        >
          {{
            $t("pages.settings.application.nades.import.errors_truncated", {
    ...
    <div class="space-y-1.5 rounded-md border border-border bg-foreground/5 p-3">
      ...
      >
        {{ $t("pages.settings.application.nades.import.grading") }}
      </div>
    ...
    <div class="space-y-2 pt-1">
      <div class="flex flex-wrap items-center justify-between gap-2">
        <p class="min-w-0 flex-1 text-xs text-muted-foreground">
          {{ $t("pages.settings.application.nades.import.purge_description") }}
        </p>
        <Button variant="outline" size="sm" @click="previewPurge()">
          <Eye class="mr-1 h-4 w-4" />
          {{ $t("pages.settings.application.nades.import.purge_preview") }}
        </Button>
        ...
        >
          ...
          {{ $t("pages.settings.application.nades.import.purge") }}
        </Button>
      </div>
      ...
      >
        {{
          purgePreview.lineups > 0
            ? $t(
                "pages.settings.application.nades.import.purge_preview_result",
                { count: purgePreview.lineups },
              )
            : $t("pages.settings.application.nades.import.purge_preview_empty")
        }}
      </p>
    </div>
    ...
    <AlertDialog :open="confirmPurge" @update:open="confirmPurge = $event">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            {{
              $t("pages.settings.application.nades.import.confirm_purge", {
          ...
          <AlertDialogDescription>
            {{
              $t(
                "pages.settings.application.nades.import.confirm_purge_description",
              )
            }}
          </AlertDialogDescription>
        </AlertDialogHeader>
