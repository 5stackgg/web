...
import {
  ...
  createNadeCollectionMutation,
  nadeCollectionItemsQuery,
  nadeCollectionsQuery,
  removeLineupFromCollectionMutation,
} from "~/graphql/nadesGraphql";
import { order_by } from "~/generated/zeus";
import { useAuthStore } from "~/stores/AuthStore";
import type { NadeCollection } from "~/types/nade";
...
async function load() {
  ...
  try {
    ...
    const [collectionsResult, membershipResult] = await Promise.all([
      ...
      lineupId
        ? client.query({
            query: nadeCollectionItemsQuery,
            variables: {
              where: { nade_lineup_id: { _eq: lineupId } },
            },
            fetchPolicy: "network-only",
          })
        : Promise.resolve({ data: null }),
    ]);
    collections.value = (collectionsResult.data as any)?.nade_collections ?? [];
    memberOf.value = new Set(
      ((membershipResult.data as any)?.nade_collection_items ?? []).map(
        (row: { collection_id: string }) => row.collection_id,
      ),
    );
  } catch (error: any) {
    toast({
      title: t("pages.nades.collections.load_failed"),
...
async function toggle(collection: NadeCollection) {
  ...
  try {
    ...
    toast({
      title: t("pages.nades.collections.update_failed"),
      description: error?.message,
      variant: "destructive",
    });
  }
...
async function create() {
  ...
  try {
    ...
    const { data } = await client.mutate({
      mutation: createNadeCollectionMutation,
      variables: {
        object: { name },
      },
    });
    const id = (data as any)?.insert_nade_collections_one?.id;
    if (!id) {
    ...
    await load();
    toast({
      title: t("pages.nades.collections.created", { name }),
    });
  } catch (error: any) {
    toast({
      title: t("pages.nades.collections.create_failed"),
      description: error?.message,
      variant: "destructive",
    });
  }
  ...
  <Popover v-if="mySteamId" v-model:open="open">
    <PopoverTrigger as-child>
      <Button variant="outline" size="sm">
        ...
        {{ chosenName ?? $t("pages.nades.collections.add_to") }}
      </Button>
    </PopoverTrigger>
    <PopoverContent align="end" class="w-[min(92vw,320px)] p-2">
      ...
      <template v-else>
        ...
        <p v-else class="px-2 py-3 text-xs text-muted-foreground">
          {{ $t("pages.nades.collections.empty") }}
        </p>
        ...
        <div class="flex items-center gap-1.5">
          <Input
            v-model="newName"
            class="h-8 text-xs"
            :placeholder="$t('pages.nades.collections.new_placeholder')"
          ...
          <Button
            ...
            :title="$t('pages.nades.collections.create')"
            @click="create()"
          >
            <Plus class="h-4 w-4" />
          </Button>
        ...
        >
          ...
          {{
            chooserMode
              ? $t("pages.nades.collections.hint_choose")
              : $t("pages.nades.collections.hint")
          }}
        </p>
      </template>
    </PopoverContent>
