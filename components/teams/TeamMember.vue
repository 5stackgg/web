  ...
  <div class="flex items-center space-x-4">
    ...
    <div>
      <p class="text-sm font-medium leading-none">
        {{ member.player.name }} <pre>{{ memberRole }}</pre>
      </p>
    ...
    <PopoverContent class="p-0" align="end">
      <Command v-model="memberRole">
        ...
        <CommandList>
          ...
          <CommandGroup>
            <CommandItem :value="role.value" class="flex flex-col items-start px-4 py-2" v-for="role of roles">
              <p>{{role.value }}</p>
              <p class="text-sm text-muted-foreground">
                {{ role.description }}
              </p>
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
          <AlertDialogDescription>
            This cancel the invite to {{ member.player.name }} ({{ member.player.steam_id }}).
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>Cancel</AlertDialogCancel>
