...
import { ArrowUpRight, Info, Play, Radar, Wrench } from "lucide-vue-next";
...
import NadeRepairDialog from "~/components/nades/NadeRepairDialog.vue";
import TimeAgo from "~/components/TimeAgo.vue";
import { toast } from "~/components/ui/toast";
import getGraphqlClient from "~/graphql/getGraphqlClient";
import { generateQuery } from "~/graphql/graphqlGen";
import {
  nadeDriftResultsQuery,
  nadeDriftScansQuery,
  nadeLineupsQuery,
  startNadeDriftScanMutation,
...
import {
  ...
  humanizeNadeToken,
  nadeDriftReasonKey,
  nadeDriftSeverityTone,
  nadeDriftVerdictKey,
  nadeDriftVerdictTone,
  sortNadeDriftResults,
} from "~/utilities/nadeDisplay";
import { readNadeDriftResult, readNadeDriftScan } from "~/types/nade";
import type {
  NadeDriftResult,
  NadeDriftResultView,
  NadeDriftScan,
...
const scans = ref<NadeDriftScanView[]>([]);
const loadingScans = ref(true);
const selectedScanId = ref<string | null>(null);
const results = ref<NadeDriftResultView[]>([]);
...
async function loadMaps() {
  try {
    ...
    console.error("[nades] drift map load error:", error);
    maps.value = [];
  }
}
...
async function loadScans() {
  ...
  try {
    ...
    scans.value = (((data as any)?.nade_drift_scans ?? []) as NadeDriftScan[])
      .map(readNadeDriftScan);
    if (!selectedScanId.value && scans.value.length) {
      selectedScanId.value = scans.value[0].id;
    }
  } catch (error) {
    console.error("[nades] drift scan load error:", error);
    scans.value = [];
  } finally {
...
async function loadResults(scanId: string) {
  ...
  try {
    ...
    const { data } = await client.query({
      query: nadeDriftResultsQuery,
      variables: {
        where: { nade_drift_scan_id: { _eq: scanId } },
    ...
    const rows = (((data as any)?.nade_drift_results ?? []) as NadeDriftResult[])
      .map(readNadeDriftResult);
    ...
    console.error("[nades] drift result load error:", error);
    results.value = [];
  } finally {
...
const verdictOptions = computed(() => {
  ...
  for (const verdict of NADE_DRIFT_VERDICTS) {
    ...
    options.push({
      key: verdict,
      label: t(`pages.nades.drift.verdicts.${verdict}`),
      count,
    });
  }
  for (const [verdict, count] of [...loaded.entries()].sort()) {
    if ((NADE_DRIFT_VERDICTS as readonly string[]).includes(verdict)) {
      continue;
    }
    options.push({
      key: verdict,
      label: humanizeNadeToken(verdict) || t("common.na"),
      count,
    });
  }
  return options;
...
async function startScan() {
  ...
  try {
    const { data } = await getGraphqlClient().mutate({
      mutation: startNadeDriftScanMutation,
      variables: {
    ...
    const output = (data as any)?.startNadeDriftScan as
      | NadeDriftScanOutput
      | undefined;
    toast({
      title: t("pages.nades.drift.started", { count: output?.lineups ?? 0 }),
    });
    await loadScans();
    if (output?.scan_id) {
    ...
    toast({
      title: t("pages.nades.drift.start_failed"),
...
function lineupName(row: NadeDriftResultView) {
  return lineupsById.value[row.lineupId]?.name ?? null;
}

const repairOpen = ref(false);
const repairRow = ref<NadeDriftResultView | null>(null);
 ...
 * diagnosed.
 */
function canRepair(row: NadeDriftResultView) {
  return nadeDriftVerdictKey(row.verdict) === "moved";
}

function startRepair(row: NadeDriftResultView) {
  repairRow.value = row;
...
<template>
  <PageTransition>
    <TacticalPageHeader>
      <template #description>{{ $t("pages.nades.drift.eyebrow") }}</template>
      <template #title>{{ $t("pages.nades.drift.title") }}</template>
      <template #subtitle>{{ $t("pages.nades.drift.subtitle") }}</template>
      <template #actions>
    ...
    >
      ...
      <div class="min-w-0 text-xs leading-relaxed">
        <div class="font-semibold text-[hsl(var(--tac-amber))]">
          {{ $t("pages.nades.drift.framing_title") }}
        </div>
        <p class="mt-0.5 text-muted-foreground">
          {{ $t("pages.nades.drift.framing") }}
        </p>
      </div>
    </div>
  </PageTransition>
  ...
  <PageTransition :delay="80" class="mt-4">
    ...
    >
      <label class="flex min-w-[12rem] flex-col gap-1">
        ...
        >
          {{ $t("pages.nades.drift.map") }}
        </span>
        <Select v-model="scanMap">
          <SelectTrigger class="h-8 text-xs">
            <SelectValue :placeholder="$t('pages.nades.drift.map')" />
          </SelectTrigger>
          <SelectContent>
      ...
      <label class="flex flex-col gap-1">
        <span
          class="font-mono text-[0.6rem] uppercase tracking-[0.16em] text-muted-foreground"
        >
          {{ $t("pages.nades.drift.from_revision") }}
        </span>
        <Input
          v-model="fromRevision"
          class="h-8 w-40 text-xs"
          :placeholder="$t('pages.nades.drift.revision_placeholder')"
        />
      </label>
      ...
      <label class="flex flex-col gap-1">
        <span
          class="font-mono text-[0.6rem] uppercase tracking-[0.16em] text-muted-foreground"
        >
          {{ $t("pages.nades.drift.to_revision") }}
        </span>
        <Input
          v-model="toRevision"
          class="h-8 w-40 text-xs"
    ...
    <div class="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
          ...
          >
            ...
            >
              ...
              <template v-if="scan.durationSeconds !== null">
                · {{ $t("pages.nades.drift.duration", { seconds: scan.durationSeconds }) }}
              </template>
            </span>
            ...
            >
              <span v-if="scan.broken" class="text-destructive">
                {{
                  $t("pages.nades.drift.count_broken", { count: scan.broken })
                }}
              </span>
              <span v-if="scan.moved" class="text-[hsl(var(--tac-amber))]">
                {{ $t("pages.nades.drift.count_moved", { count: scan.moved }) }}
              </span>
              <span v-if="scan.unsimulatable">
                {{
                  $t("pages.nades.drift.count_unsimulatable", {
              ...
              <span v-if="scan.unchanged">
                {{
                  $t("pages.nades.drift.count_unchanged", {
              ...
              <span v-if="scan.maxDistance !== null">
                {{
                  $t("pages.nades.drift.max_distance", {
      ...
      <div class="flex flex-col gap-2">
        ...
        >
          ...
          >
            {{ $t("pages.nades.drift.requested_by") }}
          </NuxtLink>
        </div>
        ...
        <Empty v-else-if="!selectedScan">
          <EmptyTitle>{{ $t("pages.nades.drift.no_scan_selected") }}</EmptyTitle>
          <EmptyDescription>
            {{ $t("pages.nades.drift.no_scan_selected_description") }}
          </EmptyDescription>
        </Empty>

        <Empty v-else-if="!visibleResults.length">
          <EmptyTitle>{{ $t("pages.nades.drift.no_results") }}</EmptyTitle>
          <EmptyDescription>
            {{ $t("pages.nades.drift.no_results_description") }}
          </EmptyDescription>
        </Empty>
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              <span
                ...
                :class="nadeDriftVerdictTone(row.verdict)"
              >
                {{
                  nadeDriftVerdictKey(row.verdict)
                    ? $t(
                        `pages.nades.drift.verdicts.${nadeDriftVerdictKey(row.verdict)}`,
                      )
                    : humanizeNadeToken(row.verdictRaw) || $t("common.na")
                }}
              </span>
              ...
              <span
                v-if="row.severity"
                class="rounded-sm border px-1.5 py-0.5 font-mono text-[0.58rem] uppercase tracking-[0.12em]"
                :class="nadeDriftSeverityTone(row.severity)"
              >
                {{ $t(`pages.nades.drift.severities.${row.severity}`) }}
              </span>
              ...
              <span
                ...
                :title="$t('pages.nades.repair.queued_hint')"
              >
                <Wrench class="h-3 w-3" />
                {{ $t("pages.nades.repair.queued") }}
              </span>
              ...
              >
                ...
                {{ $t("pages.nades.repair.action") }}
              </Button>

              <NuxtLink
                :to="{ name: 'nades-lineup-id', params: { id: row.lineupId } }"
                ...
                "
              >
                {{ lineupName(row) ?? $t("pages.nades.drift.open_lineup") }}
                <ArrowUpRight class="h-3.5 w-3.5" />
              </NuxtLink>
            </div>

            <div v-if="row.broken && nadeDriftReasonKey(row.reason)">
              <div
                ...
                "
              >
                {{ $t(`pages.nades.drift.reasons.${row.reason}`) }}
              </div>
              <p class="mt-0.5 text-xs leading-relaxed text-muted-foreground">
                {{ $t(`pages.nades.drift.reason_notes.${row.reason}`) }}
              </p>
            </div>
            ...
            >
              {{ $t("pages.nades.drift.unsimulatable_note") }}
            </p>
            ...
            <p
              ...
              class="text-xs leading-relaxed text-muted-foreground"
            >
              {{ $t("pages.nades.drift.no_distance") }}
            </p>
          </div>
        </template>
      </div>
