...
const delta = computed(() => nadeAimDelta(props.lineup));
...
const deltaIsLarge = computed(
  () =>
    isDerived.value &&
    delta.value !== null &&
    delta.value >= NADE_AIM_DELTA_WARN_DEGREES,
);
...
<template>
  ...
  >
    ...
    <div class="min-w-0 flex-1">
      ...
      >
        {{ $t(`pages.nades.confidence.${tone.key}`) }}
      </span>
      <span
      ...
      >
        {{
          $t("pages.nades.confidence.delta_short", {
      ...
      >
        {{ $t(`pages.nades.confidence.${tone.key}_note`) }}
      </span>
      <p
      ...
      >
        {{
          $t("pages.nades.confidence.delta_detail", {
