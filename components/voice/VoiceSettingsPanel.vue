...
import { Headphones, Mic, MicOff, Play, Radio, Volume2 } from "lucide-vue-next";
import { Button } from "~/components/ui/button";
import { Switch } from "~/components/ui/switch";
import { Slider } from "~/components/ui/slider";
import DeviceSelect from "~/components/media/DeviceSelect.vue";
import type { VoiceInputMode } from "~/composables/useAudioSettings";

// The controls themselves, with no opinion about what is holding them -- the
// gear on a voice panel opens them in a dialog, the settings page renders the
// same thing inline. One microphone, one set of settings, so one panel.
const props = withDefaults(
  defineProps<{
    ...
    monitoring: boolean;
    // The microphone this panel is metering is open. Whether it is being sent
    // anywhere is the host surface's business, not this component's.
    live: boolean;
    // An i18n key. Covers both "this browser cannot" and "your microphone
    // refused", since either one makes everything below it moot.
    error?: string | null;
    errorDetail?: string | null;
    // Set when the microphone is live in a different channel. The settings still
    // apply -- they are shared -- but this panel is not the one metering.
    busyChannel?: string | null;
    // The host expects the player to open the mic themselves rather than having
    // it opened for them, so the mic check becomes a button.
    canStart?: boolean;
  }>(),
  {
    error: null,
...
const meterHint = computed(() => {
  if (props.live) {
    return props.monitoring
      ? "voice.settings.monitor_hint"
      : "voice.settings.meter_hint";
  }

  if (props.busyChannel) {
    return "voice.settings.mic_in_use";
  }

  return props.canStart
    ? "voice.settings.mic_idle"
    : "voice.settings.mic_starting";
});
</script>
...
<template>
  <div class="space-y-6">
    <div
      v-if="error"
      class="space-y-1 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs leading-relaxed text-destructive"
    >
      <p>{{ $t(error) }}</p>
      <p v-if="errorDetail" class="font-mono text-[10px] opacity-70">
        {{ errorDetail }}
      </p>
    </div>
    ...
    <section class="space-y-2.5">
      <div class="flex items-center justify-between gap-3">
        ...
        <Button
          v-if="canStart && !live && !busyChannel"
          size="xs"
          variant="outline"
        ...
        >
          <component :is="monitoring ? Headphones : Play" class="h-3 w-3" />
          {{
            monitoring
      ...
      <p class="text-[11px] leading-relaxed text-muted-foreground">
        {{ $t(meterHint, { channel: busyChannel }) }}
      </p>
    </section>
