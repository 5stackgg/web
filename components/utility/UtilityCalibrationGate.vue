...
import type { UtilityCalibrationView } from "~/types/utility";

defineProps<{
  calibration: UtilityCalibrationView | null;
  checking: boolean;
}>();
...
<template>
  ...
  >
    ...
    <span class="text-sm">{{ $t("pages.utility.solve.checking") }}</span>
  </div>
  ...
  >
    <div class="text-sm font-medium">
      {{ $t(`pages.utility.solve.state.${calibration.state}`) }}
    </div>
    <p class="text-xs leading-relaxed text-muted-foreground">
      {{ $t(`pages.utility.solve.state_note.${calibration.state}`) }}
    </p>
    <p
    ...
    <div class="flex items-center gap-2 pt-1">
      ...
      >
        ...
        {{ $t("pages.utility.solve.check_again") }}
      </Button>
    </div>
  </div>
</template>
