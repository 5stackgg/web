...
import type { NadeLineup, NadeTrajectoryPoint } from "~/types/nade";
...
// code path it renders a demo with. Nothing about the renderer is nade-aware.
...
const replayType = computed(() => replayNadeType(props.lineup.nade_type));
const throwerTeam = computed(() => replayTeamForSide(props.lineup.side));

const fullTrajectory = ref<NadeTrajectoryPoint[] | null>(null);
const smokeVolume = ref<SmokeVolume | null>(null);

const trajectory = computed<NadeTrajectoryPoint[]>(() => {
  ...
  const landing = nadeLanding(props.lineup);
  if (!landing) {
    return [];
  }
  return [
...
async function loadTrajectory() {
  ...
  try {
    const response = await fetch(
      `https://${apiDomain}/nades/${props.lineup.id}/trajectory`,
      { credentials: "include" },
    );
    if (!response.ok) {
      return;
...
function pointAtProgress(fraction: number): NadeTrajectoryPoint {
  ...
  >
    ...
    >
      ...
      >
        ...
        {{ $t("pages.nades.viewer.replay_throw") }}
      </button>
