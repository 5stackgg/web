...
import NadeSolvePanel from "~/components/nades/NadeSolvePanel.vue";
import NadeSaveLineupDialog from "~/components/nades/NadeSaveLineupDialog.vue";
import { toast } from "~/components/ui/toast";
import getGraphqlClient from "~/graphql/getGraphqlClient";
import {
  inviteToNadePracticeMutation,
  joinNadePracticeMutation,
  leaveNadePracticeMutation,
...
import { readNadePracticeSession } from "~/types/nade";
import type {
  NadeCollection,
  NadePlaybook,
  NadePracticeSession,
...
function subscribeSession(id: string) {
  ...
  sessionSub = getGraphqlClient()
    ...
    .subscribe({
      ...
      error: (error: unknown) => {
        console.error("[nades] practice session subscription error:", error);
      },
    });
}
...
async function loadCollections() {
  try {
    ...
    collections.value = (data as any)?.nade_collections ?? [];
  } catch (error) {
    console.error("[nades] practice collection load error:", error);
    collections.value = [];
  }
}
...
async function loadPlaybooks() {
  try {
    ...
    playbooks.value = (data as any)?.nade_playbooks ?? [];
  } catch (error) {
    console.error("[nades] practice playbook load error:", error);
    playbooks.value = [];
  }
}
  ...
  async ([isOpen, target]) => {
    ...
    try {
      const { data } = await getGraphqlClient().mutate({
        mutation: joinNadePracticeMutation,
        variables: target.variables,
      });
      const output = (data as any)?.joinNadePractice as
        | NadePracticeSessionOutput
        | undefined;
      if (!output?.id) {
      ...
      toast({
        title: t("pages.nades.practice.join_failed"),
...
async function start() {
  try {
    ...
    const output = (data as any)?.startNadePractice as
      | NadePracticeSessionOutput
      | undefined;
    if (!output?.id) {
      throw new Error("no session");
...
type NadeInvitee = { steamId: string; name: string };

const invitees = ref<NadeInvitee[]>([]);
...
async function leave() {
  ...
  try {
    await getGraphqlClient().mutate({
      mutation: leaveNadePracticeMutation,
      variables: { session_id: id },
    });
    unsubscribeSession();
    sessionId.value = null;
    ...
    toast({
      title: t("pages.nades.practice.leave_failed"),
      description: error?.message,
      variant: "destructive",
    });
  }
...
async function invitePlayers() {
  ...
  try {
    await getGraphqlClient().mutate({
      mutation: inviteToNadePracticeMutation,
      variables: {
        session_id: id,
    ...
    toast({
      title: t("pages.nades.practice.invited", {
    ...
    toast({
      title: t("pages.nades.practice.invite_failed"),
      description: error?.message,
      variant: "destructive",
    });
  } finally {
...
async function applyPlaybook(id: string | null) {
  ...
  try {
    await getGraphqlClient().mutate({
      mutation: loadNadePlaybookIntoSessionMutation,
      variables: { session_id: session, playbook_id: id },
    });
  } catch (error: any) {
    toast({
      title: t("pages.nades.playbooks.load_failed"),
      description: error?.message,
      variant: "destructive",
    });
  }
    ...
    <DialogContent :class="mode === 'solve' ? 'sm:max-w-3xl' : 'sm:max-w-lg'">
      <DialogHeader>
        <DialogTitle class="flex items-center gap-2">
          ...
          {{ $t("pages.nades.practice.title") }}
        </DialogTitle>
        <DialogDescription>
          {{ $t("pages.nades.practice.description") }}
        </DialogDescription>
      </DialogHeader>
      ...
      <div v-else class="space-y-4">
        <div v-if="!sessionId" class="space-y-2">
          ...
          >
            ...
            {{ $t("pages.nades.practice.collection") }}
          </span>
          <Select v-model="collectionId">
            ...
            <SelectContent>
              <SelectItem :value="NO_COLLECTION">
                {{ $t("pages.nades.practice.no_collection") }}
              </SelectItem>
              <SelectItem
          ...
          <template v-if="playbooks.length">
            ...
            >
              ...
              {{ $t("pages.nades.playbooks.load_label") }}
            </span>
          ...
          >
            <span class="min-w-0">
              <span class="block text-sm font-medium">
                {{ $t("pages.nades.practice.open_session") }}
              </span>
              <span class="block text-xs text-muted-foreground">
                {{ $t("pages.nades.practice.open_session_hint") }}
              </span>
            </span>
            <Switch
              :model-value="isOpen"
          ...
          <template v-if="canDriveSession">
            ...
            <div class="space-y-2">
              ...
              >
                ...
                {{ $t("pages.nades.practice.invite_players") }}
              </div>
              <p class="text-xs text-muted-foreground">
                {{ $t("pages.nades.practice.invite_players_hint") }}
              </p>

              <PlayerSearch
                :label="$t('pages.nades.practice.pick_players')"
              ...
              >
                ...
                {{
                  $t("pages.nades.practice.send_invites", {
        ...
        >
          ...
          <p v-if="loadedPlaybook" class="text-xs text-muted-foreground">
            {{
              $t("pages.nades.playbooks.loaded", { name: loadedPlaybook.name })
            }}
          </p>
          <div class="flex items-center gap-2">
            ...
            >
              {{
                playbookChoice === NO_PLAYBOOK
                  ? $t("pages.nades.playbooks.unload")
                  : $t("pages.nades.playbooks.load")
              }}
            </Button>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            {{ $t("pages.nades.solve.title") }}
          </span>
          <p class="text-xs text-muted-foreground">
            {{ $t("pages.nades.solve.entry_hint") }}
          </p>
          <Button variant="outline" @click="mode = 'solve'">
            {{ $t("pages.nades.solve.open") }}
          </Button>
        </div>
        ...
        >
          ...
          >
            ...
            {{ $t("pages.nades.save.practice_entry_title") }}
          </span>
          <p class="text-xs text-muted-foreground">
            {{ $t("pages.nades.save.practice_entry_hint") }}
          </p>
          <Button variant="outline" @click="saveFromPracticeOpen = true">
            {{ $t("pages.nades.save.practice_entry_open") }}
          </Button>
        </div>
      </div>
