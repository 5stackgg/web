...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    >
      <div class="font-medium text-base mb-3 truncate" :title="team.name">{{ team.name }}</div>
      ...
      >
        <NuxtLink
          ...
          :to="{ name: 'players-id', params: { id: rosterItem.player?.steam_id } }"
        ...
        >
          ...
          <div class="flex items-center justify-center gap-0.5 min-h-3 w-full overflow-hidden">
            <TimezoneFlag
              v-if="rosterItem.player?.country"
              :country="rosterItem.player.country"
              class="shrink-0 h-3 w-4"
            />
            <span class="text-[10px] font-medium truncate max-w-[32px]" :title="rosterItem.player?.name">
      ...
      >
        {{ team.roster?.length }} {{ $t("team.roster_count_players") || "players" }}
      </p>
    </NuxtLink>
  </div>
</template>
