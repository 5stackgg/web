...
const CANCEL_MUTATION = gql`
  mutation CancelRefreshAllPlayers {
    cancelRefreshAllPlayers {
      success
    }
  }
`;
...
const STATUS_MUTATION = gql`
  mutation RefreshAllPlayersStatus {
    refreshAllPlayersStatus {
      running
      canceled
      started_at
      finished_at
...
async function refreshStatus(): Promise<ReindexStatus | null> {
  if (!client) {
    return null;
  }
  ...
  const next = ((data as any)?.refreshAllPlayersStatus ??
    null) as ReindexStatus | null;
  status.value = next;
  loaded.value = true;
  return next;
}

function notifyComplete(s: ReindexStatus) {
  toast({
    title: s.canceled
      ? tr("pages.settings.application.players.reindex_canceled_title")
      : tr("pages.settings.application.players.reindex_complete_title"),
    description: tr(
      "pages.settings.application.players.reindex_complete_desc",
...
async function startReindex() {
  ...
  try {
    ...
    toast({
      title: tr("pages.settings.application.players.refresh_queued"),
    });
    await refreshStatus();
    poll();
  } catch (error) {
    toast({
      title: (error as Error)?.message ?? "reindex failed",
      variant: "destructive",
    });
  } finally {
...
export function useReindexPlayers() {
  if (!client) {
