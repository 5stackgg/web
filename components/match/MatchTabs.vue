    ...
    <TabsContent value="server">
      ...
      <rcon-commander :server-id="match.server_id" v-slot="{ commander }">
        <template v-for="command of availableCommands">
            ...
            <form @submit.prevent="handleCommand(commander, command)">
              <FormField v-slot="{ componentField }" name="round">
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="number" @update:model-value="(value) => form.setFieldValue('round', value)" v-bind="componentField"/>
                  </FormControl>
                </FormItem>
          ...
          <template v-else>
            <Button @click="handleCommand(commander, command)">{{ command.display}}</Button>
          </template>
        </template>
      </rcon-commander>
    </TabsContent>
