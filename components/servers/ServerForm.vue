      ...
      <FormItem>
        ...
        <FormControl>
          ...
          <FormDescription v-if="server">
            We dont display the rcon password for security. You may update the RCON password and it will replace the current one.
          </FormDescription>
        </FormControl>
      </FormItem>
    </FormField>
    ...
    return {
      form: useForm({
        validationSchema: toTypedSchema(
            z.object({
              host: z.string().ip({ version: "v4" }).or(z.string().regex(/^(?!:\/\/)(?:[-A-Za-z0-9]+\.)+[A-Za-z]{2,6}$/)),
              label: z.string().min(3),
              port: z.number().min(2).max(65535),
              tv_port: z.number().min(2).max(65535),
              rcon_password: this.server ? z.string().min(1).optional() : z.string().min(1)
            })
        )
      }),
    }
