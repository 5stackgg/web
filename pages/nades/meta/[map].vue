...
import type { NadeMetaSpot } from "~/utilities/nadeDisplay";
import type { NadeLineup, NadeMetaLineup, NadeType } from "~/types/nade";
...
async function load() {
  ...
  try {
    ...
    spots.value = toNadeMetaSpots(
      ((metaResult.data as any)?.nade_meta_lineups ?? []) as NadeMetaLineup[],
    );
    lineups.value = ((lineupResult.data as any)?.nade_lineups ??
      []) as NadeLineup[];
  } catch (error) {
    console.error("[nades] meta page load error:", error);
    spots.value = [];
    lineups.value = [];
  } finally {
    loading.value = false;
...
const visibleSpots = computed(() =>
  spots.value.filter((spot) => {
    if (types.value.length && !types.value.includes(spot.nadeType)) {
      return false;
    }
...
const lineupsBySpot = computed(() => {
  const grouped: Record<string, NadeLineup[]> = {};
  for (const lineup of lineups.value) {
    const spot = matchNadeMetaSpot(lineup, spots.value);
    if (spot) {
...
const rows = computed(() =>
  visibleSpots.value.map((spot, index) => ({
    spot,
    rank: index + 1,
    color: NADE_TYPE_COLORS[spot.nadeType] ?? "#ffffff",
    typeKey: `pages.nades.types.${spot.nadeType}`,
    sideKey: spot.side ? `pages.nades.sides.${spot.side}` : "",
    techniqueKey: spot.technique
      ? `pages.nades.techniques.${spot.technique}`
      : "",
    strengthKey: spot.throwStrength
      ? `pages.nades.strengths.${spot.throwStrength}`
      : "",
...
<template>
  <PageTransition>
    <TacticalPageHeader>
      <template #description>{{ $t("pages.nades.meta.eyebrow") }}</template>
      <template #title>{{ mapTitle }}</template>
      <template #subtitle>
        {{
          $t("pages.nades.meta.subtitle", {
  ...
  <PageTransition v-else-if="!spots.length" :delay="80" class="mt-4">
    <Empty>
      <EmptyTitle>{{ $t("pages.nades.meta.empty") }}</EmptyTitle>
      <EmptyDescription>
        {{ $t("pages.nades.meta.empty_description") }}
      </EmptyDescription>
    </Empty>
  </PageTransition>
        ...
        >
          <div class="flex items-center gap-2">
            ...
            >
              ...
              {{
                $t("pages.nades.meta.throwers", { count: row.spot.throwers })
              }}
            </span>
          </div>
          <div
          ...
          >
            <span>
              {{
                row.spot.lineups
                  ? $t("pages.nades.meta.has_lineups", {
                      count: row.spot.lineups,
                    })
                  : $t("pages.nades.meta.no_lineups")
              }}
            </span>
  ...
  <PageTransition v-if="selectedRow" :delay="100" class="mt-4">
    <div class="flex flex-col gap-3">
      ...
      >
        <div>
          <div class="text-muted-foreground">
            {{ $t("pages.nades.meta.classification") }}
          </div>
        ...
        <div>
          <div class="flex items-center gap-1 text-muted-foreground">
            ...
            {{ $t("pages.nades.meta.usage") }}
          </div>
          <div class="mt-0.5 font-mono tabular-nums">
            {{
              $t("pages.nades.meta.usage_detail", {
      ...
      >
        {{ $t("pages.nades.meta.matching_lineups") }}
      </h2>
      <Empty v-if="!selectedRow.spot.lineups">
        <EmptyTitle>{{ $t("pages.nades.meta.no_lineups") }}</EmptyTitle>
        <EmptyDescription>
          {{ $t("pages.nades.meta.no_lineups_description") }}
        </EmptyDescription>
      </Empty>
      <template v-else>
        <div
