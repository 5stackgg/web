...
const peaks = [
  ...
  { cx: 20, cy: 440, h: 0.5, sx: 80, sy: 150, rot: 0.2 },
  { cx: 1590, cy: 540, h: 0.5, sx: 85, sy: 150, rot: -0.2 },
  { cx: 720, cy: 10, h: 0.46, sx: 170, sy: 80 },
  { cx: 1000, cy: 995, h: 0.46, sx: 160, sy: 80 },
  { cx: 1300, cy: 5, h: 0.4, sx: 120, sy: 70 },
  { cx: 380, cy: 998, h: 0.4, sx: 130, sy: 70 },
  { cx: 1595, cy: 800, h: 0.4, sx: 80, sy: 110 },
  { cx: 15, cy: 880, h: 0.38, sx: 90, sy: 100 },
...
for (let li = 0; li < levels.length; li++) {
  ...
  for (const p of stitch(segs)) {
    ...
    const step = Math.max(1, Math.floor(simplified.length / 24));
    const samples = simplified.filter((_, i) => i % step === 0);
    contours.push({
...
// ── Pick animated flow paths (farthest-point by path proximity) ────────
const FLOW_COUNT = 12;
const MIN_GAP = 85;
...
while (picked.length < FLOW_COUNT && picked.length < eligible.length) {
  ...
  for (const c of eligible) {
    ...
    for (const p of picked) {
      const d = minPolyDist(c.samples, p.samples);
  ...
  if (!best || bestMinDist < MIN_GAP) break;
  picked.push(best);
}
...
const flowPaths = picked.map((c) => {
  const len = round1(c.length);
  const dash = round1(Math.min(Math.max(len * 0.26, 150), 380, len * 0.5));
  const gap = round1(len - dash);
  const dur = round1(Math.min(Math.max(len / SPEED, 22), 48));
  const delay = round1(-((c.idx * 3.7) % dur));
  return { d: c.d, dash, gap, flow: len, dur, delay };
});
