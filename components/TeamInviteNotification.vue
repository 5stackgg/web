  ...
  <div>
    ...
    <div class="flex justify-between space-x-2 mt-3">
      ...
      <div class="flex gap-2">
        <Button variant="outline" @click="denyInvite(invite.id)">{{ $t('team.invite.deny') }}</Button>
        <Button variant="default" @click="acceptInvite(invite.id)">{{ $t('team.invite.accept') }}</Button>
      </div>
    </div>
  </div>
</template>
