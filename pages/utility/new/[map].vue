...
async function save() {
  ...
  try {
    ...
    await router.push({ name: "utility-lineup-id", params: { id } });
  } catch (error: any) {
    toast({
...
<template>
  <PageTransition>
    <TacticalPageHeader>
      <template #description>{{ $t("pages.utility.create.eyebrow") }}</template>
      <template #title>{{ $t("pages.utility.create.title") }}</template>
      <template #subtitle>
        {{ $t("pages.utility.create.subtitle", { map: mapTitle }) }}
      </template>
      <template #actions>
