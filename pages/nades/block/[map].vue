...
const from = ref<NadeSightlinePoint | null>(null);
const to = ref<NadeSightlinePoint | null>(null);
const side = ref<string>(ANY_SIDE);
const types = ref<NadeType[]>([]);
const heightInput = ref(String(NADE_EYE_HEIGHT_UNITS));
const searching = ref(false);
const searched = ref(false);
const results = ref<NadeBlockingResult[]>([]);
const notice = ref<NadeAnalysisNotice | null>(null);
...
const sideOptions = computed(() => [
  ...
  ...NADE_SIDES.map((entry) => ({
    key: entry,
    label: t(`pages.nades.sides.${entry}`),
  })),
]);
...
async function seedHeight() {
  try {
    ...
    const heights = (((data as any)?.nade_lineups ?? []) as NadeLineup[])
    ...
    console.error("[nades] block height seed error:", error);
  }
}
...
const rankedLineups = computed<
  Array<{ lineup: NadeLineup; view: NadeSightlineView }>
>(() => {
  const out: Array<{ lineup: NadeLineup; view: NadeSightlineView }> = [];
  for (const result of results.value) {
    const lineup = lineupsById.value[result.nade_lineup_id];
    if (!lineup) {
      continue;
    }
    if (types.value.length && !types.value.includes(lineup.nade_type)) {
      continue;
    }
    out.push({
      lineup,
...
async function search() {
  ...
  try {
    ...
    const { data } = await client.query({
      query: findNadeLineupsBlockingQuery,
      variables: {
    ...
    const output = (data as any)?.findNadeLineupsBlocking as
      | NadeBlockingOutput
      | undefined;
    notice.value = readNadeAnalysisNotice(output);
    ...
    const { data: rows } = await client.query({
      ...
      variables: {
        where: {
          id: { _in: hits.map((hit) => hit.nade_lineup_id) },
          can_view: { _eq: true },
        },
        order_by: [{ upvotes: order_by.desc }],
        limit: hits.length,
    ...
    for (const lineup of ((rows as any)?.nade_lineups ?? []) as NadeLineup[]) {
    ...
    searched.value = true;
    toast({
      title: t("pages.nades.block.failed"),
...
function selectLineup(id: string | null) {
  ...
  document
    .getElementById(`nade-block-${selectedId.value}`)
...
<template>
  <PageTransition>
    <TacticalPageHeader>
      <template #description>{{ $t("pages.nades.block.eyebrow") }}</template>
      <template #title>{{ $t("pages.nades.block.title") }}</template>
      <template #subtitle>
        {{ $t("pages.nades.block.subtitle", { map: mapTitle }) }}
      </template>
      <template #actions>
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_380px]">
      <div class="lg:sticky lg:top-4 lg:self-start">
        ...
        <p class="mt-2 flex items-start gap-1.5 text-xs text-muted-foreground">
          ...
          {{
            from && !to
              ? $t("pages.nades.block.pending_hint")
              : $t("pages.nades.block.hint")
          }}
        </p>
        ...
        <div class="mt-2 flex flex-wrap items-center gap-2">
          ...
          >
            {{ $t("pages.nades.block.height_label") }}
            <Input
              v-model="heightInput"
              type="number"
      ...
      <div class="flex flex-col gap-2">
        ...
        <Empty v-else-if="!searched">
          <EmptyTitle>{{ $t("pages.nades.block.empty") }}</EmptyTitle>
          <EmptyDescription>
            {{ $t("pages.nades.block.empty_description") }}
          </EmptyDescription>
        </Empty>
        ...
        <Empty v-else-if="!rankedLineups.length && notice?.degraded">
          <EmptyTitle>{{ $t("pages.nades.block.degraded") }}</EmptyTitle>
          <EmptyDescription>
            {{ $t("pages.nades.block.degraded_description") }}
            <span
              v-if="notice.message"
          ...
          >
            <Search class="mr-1 h-4 w-4" />
            {{ $t("pages.nades.block.retry") }}
          </Button>
        </Empty>

        <Empty v-else-if="!rankedLineups.length">
          <EmptyTitle>{{ $t("pages.nades.block.no_results") }}</EmptyTitle>
          <EmptyDescription>
            {{ $t("pages.nades.block.no_results_description") }}
          </EmptyDescription>
        </Empty>
        ...
        <template v-else>
          ...
          >
            ...
            <div class="min-w-0 text-[0.7rem] leading-snug">
              ...
              >
                {{ $t("pages.nades.block.degraded") }}
              </div>
              <p class="text-muted-foreground">
                {{ $t("pages.nades.block.degraded_partial") }}
              </p>
            </div>
          </div>
          ...
          >
            {{
              $t("pages.nades.block.results", { count: rankedLineups.length })
            }}
          </p>

          <div
            v-for="(entry, index) of rankedLineups"
            :id="`nade-block-${entry.lineup.id}`"
            :key="entry.lineup.id"
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span
                ...
                :class="NADE_SIGHTLINE_TONES[entry.view.verdict]"
              >
                {{ $t(`pages.nades.block.coverage.${entry.view.verdict}`) }}
              </span>
              <span
              ...
              >
                ...
                ·
                {{
                  $t("pages.nades.sightline.depth", { units: entry.view.depth })
                }}
              </span>
            </div>
