...
const props = withDefaults(
  defineProps<{
    ...
    lineupsById?: Record<string, NadeLineup>;
  }>(),
  {
    steps: () => [],
    lineupsById: () => ({}),
...
const coverageByPair = ref<Record<string, NadePlaybookCoverageResult>>({});
...
const boardLineups = computed(() =>
  orderedSteps.value
    .map((step) => props.lineupsById?.[step.nade_lineup_id])
...
function viewOf(pairId: string): NadeCoverageView | null {
  if (!notice.value || !checkedIds.value.has(pairId)) {
    return null;
  }
  return readNadePlaybookCoverage(
...
function pairColor(pairId: string) {
  ...
  return view
    ? NADE_COVERAGE_COLORS[view.verdict]
    : NADE_SIGHTLINE_UNCHECKED_COLOR;
}
...
async function check() {
  ...
  try {
    const { data } = await getGraphqlClient().query({
      query: analyseNadePlaybookCoverageQuery,
      variables: {
    ...
    const output = (data as any)?.analyseNadePlaybookCoverage as
      | NadePlaybookCoverageOutput
      | undefined;
    notice.value = readNadeAnalysisNotice(output);
    const next: Record<string, NadePlaybookCoverageResult> = {};
    ...
    checkedIds.value = new Set(sent);
    toast({
      title: t("pages.nades.coverage.failed"),
...
function resolveStep(byStep: number | null) {
  ...
  return {
    ...
    stepName: props.lineupsById?.[step.nade_lineup_id]?.name ?? null,
  };
}
  ...
  >
    <div class="flex items-start justify-between gap-2">
      <div class="min-w-0">
        ...
        >
          {{ $t("pages.nades.coverage.eyebrow") }}
        </div>
        <h2 class="text-sm font-semibold">
          {{ $t("pages.nades.coverage.title") }}
        </h2>
        <p class="mt-0.5 text-xs text-muted-foreground">
          {{ $t("pages.nades.coverage.description") }}
        </p>
      </div>
      <Button
        variant="outline"
      ...
      >
        ...
        {{
          open ? $t("pages.nades.coverage.hide") : $t("pages.nades.coverage.open")
        }}
      </Button>
    </div>
    ...
    >
      <div class="flex flex-col gap-2">
        ...
        <p class="flex items-start gap-1.5 text-xs text-muted-foreground">
          ...
          {{
            pending
              ? $t("pages.nades.coverage.pending_hint")
              : $t("pages.nades.coverage.hint")
          }}
        </p>
      ...
      <div class="flex flex-col gap-3">
        ...
        <p class="text-[0.7rem] leading-snug text-muted-foreground">
          {{ $t("pages.nades.coverage.saved_only") }}
        </p>
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            ...
            >
              {{ $t("pages.nades.coverage.degraded") }}
            </div>
            <p class="mt-0.5 text-[0.7rem] leading-snug text-muted-foreground">
              {{ $t("pages.nades.coverage.degraded_note") }}
            </p>
            <p
              v-if="notice.message"
        ...
        >
          <span>
            {{ $t("pages.nades.coverage.open_count", { count: openCount }) }}
          </span>
          <span v-if="unknownCount">
            {{
              $t("pages.nades.coverage.unknown_count", { count: unknownCount })
            }}
          </span>
        </div>
        ...
        >
          ...
          <template v-if="row.view">
            <div
              ...
              :class="NADE_COVERAGE_TONES[row.view.verdict]"
            >
              <div class="font-mono text-[0.62rem] uppercase tracking-[0.12em]">
                {{ $t(`pages.nades.coverage.verdict.${row.view.verdict}`) }}
              </div>
              <p class="mt-0.5 text-[0.7rem] leading-snug opacity-90">
                {{ $t(`pages.nades.coverage.verdict_note.${row.view.verdict}`) }}
              </p>
              <p
              ...
              >
                {{
                  row.stepName
                    ? $t("pages.nades.coverage.by_step_named", {
                        step: row.stepPosition,
                        name: row.stepName,
                      })
                    : $t("pages.nades.coverage.by_step", {
                        step: row.stepPosition,
                      })
                }}
              </p>
