...
const { calibration, checking, refresh: refreshCalibration } =
  useNadeSolverCalibration(() => props.sessionId, {
    onError: (error: any) => {
      toast({
...
const target = ref<NadeSightlinePoint | null>(null);
const throwFrom = ref<NadeSightlinePoint | null>(null);
const pickMode = ref<"target" | "from">("target");
const utilityType = ref<NadeType>("Smoke");
const lineupName = ref("");
const toleranceInput = ref("");
const heightInput = ref(String(NADE_EYE_HEIGHT_UNITS));

const submitting = ref(false);
const solveOutput = ref<NadeSolveOutput | null>(null);
const waiting = ref(false);
const waitedMs = ref(0);
const timedOut = ref(false);
const cancelled = ref(false);
const solved = ref<NadeLineup | null>(null);
...
const markers = computed<NadeBoardMarker[]>(() => {
  ...
  if (throwFrom.value) {
    out.push({
      ...
      label: t("pages.nades.solve.from_short"),
    });
  }
  if (target.value) {
    out.push({
      key: "target",
      point: target.value,
      color: NADE_TYPE_COLORS[utilityType.value],
      label: t("pages.nades.solve.target_short"),
    });
  }
  return out;
});
...
const progress = computed(() =>
  Math.min(100, Math.round((waitedMs.value / NADE_SOLVE_TIMEOUT_MS) * 100)),
);

const waitedSeconds = computed(() => Math.floor(waitedMs.value / 1000));

const timeoutSeconds = Math.round(NADE_SOLVE_TIMEOUT_MS / 1000);
...
async function lookForSolvedLineup(since: string) {
  ...
  try {
    ...
    const match = rows.find((lineup) => nadeSolveMatches(lineup, wanted));
    if (match) {
      solved.value = match;
      stopWatching();
    }
  } catch (error) {
    console.error("[nades] solve watch error:", error);
  }
}
...
function watchForLineup(since: string, startedAt: number) {
  ...
  const tick = () => {
    lineupTimer = setTimeout(async () => {
      ...
      if (Date.now() - startedAt >= NADE_SOLVE_TIMEOUT_MS) {
        timedOut.value = true;
        stopWatching();
        return;
      }
...
async function solve() {
  ...
  try {
    ...
    const { data } = await getGraphqlClient().mutate({
      mutation: solveNadeLineupMutation,
      variables: {
    ...
    const output = (data as any)?.solveNadeLineup as
      | NadeSolveOutput
      | undefined;
    ...
    toast({
      title: t("pages.nades.solve.failed"),
      description: error?.message,
      variant: "destructive",
    });
  } finally {
...
<template>
  <div class="space-y-4">
    ...
    <template v-else>
      <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_260px]">
        <div class="flex flex-col gap-2">
          ...
          <p class="flex items-start gap-1.5 text-xs text-muted-foreground">
            ...
            {{
              pickMode === "from"
                ? $t("pages.nades.solve.pick_from_hint")
                : $t("pages.nades.solve.pick_target_hint")
            }}
          </p>
        </div>
        ...
        <div class="flex flex-col gap-2">
          ...
          <Select v-model="utilityType">
            ...
            <SelectContent>
              <SelectItem v-for="entry of NADE_TYPES" :key="entry" :value="entry">
                {{ $t(`pages.nades.types.${entry}`) }}
              </SelectItem>
            </SelectContent>
          </Select>
          ...
          >
            {{ $t("pages.nades.solve.name") }}
          </span>
          <Input
            v-model="lineupName"
            :placeholder="$t('pages.nades.solve.name_placeholder')"
            class="h-8 text-xs"
          />
          ...
          <span
            class="pt-1 font-mono text-[0.62rem] uppercase tracking-[0.18em] text-muted-foreground"
          >
            {{ $t("pages.nades.solve.tolerance") }}
          </span>
          <Input
            v-model="toleranceInput"
            type="number"
            :placeholder="$t('pages.nades.solve.tolerance_placeholder')"
            class="h-8 text-xs"
          />
          ...
          >
            {{
              throwFrom
                ? $t("pages.nades.solve.repick_from")
                : $t("pages.nades.solve.pick_from")
            }}
          </Button>
      ...
      >
        {{ $t("pages.nades.solve.cancelled_note") }}
      </p>
      ...
      <div v-if="solved" class="space-y-2">
        ...
        >
          ...
          {{ $t("pages.nades.solve.solved") }}
        </div>
        <NadeLineupCard :lineup="solved" />
      </div>
    </template>
  </div>
</template>
