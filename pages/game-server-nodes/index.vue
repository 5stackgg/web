...
<template>
  <div class="flex-grow flex flex-col gap-4">
    <PageHeading>
      ...
      <template #description>{{ $t('pages.game_server_nodes.description') }}</template>
      <template #actions>
        <Popover>
          <PopoverTrigger class="flex gap-4">
            <template v-if="!supportsGameServerNodes">
              <Alert class="bg-background text-lg">
                <Info class="h-4 w-4" />
                <AlertTitle>{{ $t('pages.game_server_nodes.not_supported.title') }}</AlertTitle>
                <AlertDescription>
            ...
            >
              ...
              <span class="hidden md:inline ml-2">{{ $t('pages.game_server_nodes.create') }}</span>
            </Button>
          </PopoverTrigger>
          <PopoverContent>
    ...
    <Alert class="flex items-center gap-4">
      <div class="flex items-center gap-2">
        <Info class="h-4 w-4" />
        <AlertTitle class="m-0">{{ $t('pages.game_server_nodes.cs_version_info') }}</AlertTitle>
      </div>
      <AlertDescription class="m-0 flex items-center gap-2">
        <span>{{ $t('pages.game_server_nodes.build_id', { id: csVersion }) }}</span>
        <span class="text-muted-foreground">•</span>
        <span>{{ $t('pages.game_server_nodes.last_updated', { date: csVersionLastUpdated?.toLocaleString() }) }}</span>
      </AlertDescription>
    </Alert>
    ...
    <Card class="p-4">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>{{ $t('pages.game_server_nodes.table.node') }}</TableHead>
            <TableHead>{{ $t('pages.game_server_nodes.table.lan_public_ip') }}</TableHead>
            <TableHead>{{ $t('pages.game_server_nodes.table.cs_build_id') }}</TableHead>
            <TableHead>{{ $t('pages.game_server_nodes.table.region') }}</TableHead>
            <TableHead>{{ $t('pages.game_server_nodes.table.capacity') }}</TableHead>
            <TableHead>
              <div class="flex items-center gap-1">
                {{ $t('pages.game_server_nodes.table.ports') }}
                <FiveStackToolTip>{{ $t('pages.game_server_nodes.table.ports_tooltip') }}</FiveStackToolTip>
              </div>
            </TableHead>
            <TableHead>{{ $t('pages.game_server_nodes.table.enabled') }}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          <template v-if="gameServerNodes?.length === 0">
            <TableRow>
              <TableCell colspan="5" class="text-center"
                >{{ $t('pages.game_server_nodes.table.no_nodes') }}</TableCell
              >
            </TableRow>
          </template>
          <template v-else>
