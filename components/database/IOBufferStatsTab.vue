<template>
  <div class="space-y-4">
    <Tabs default-value="table" class="w-full">
      <TabsList>
        <TabsTrigger value="table">{{ $t('pages.database.io.tabs.table_io') }}</TabsTrigger>
        <TabsTrigger value="index">{{ $t('pages.database.io.tabs.index_io') }}</TabsTrigger>
      </TabsList>

      <TabsContent value="table" class="space-y-6">
        <!-- Table I/O Stats -->
        <div>
          <h3 class="text-lg font-semibold mb-3">{{ $t('pages.database.io.table_io_stats') }}</h3>
          <Card>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            <TableRow v-for="table in filteredTableIOStats.slice(0, 30)" :key="`${table.schemaname}.${table.relname}`">
              <TableCell class="text-xs">{{ table.schemaname }}</TableCell>
              <TableCell class="font-mono text-xs">{{ table.relname }}</TableCell>
              <TableCell class="text-right">{{ table.heap_blks_hit.toLocaleString() }}</TableCell>
              ...
              <TableCell class="text-right">{{ table.idx_blks_hit.toLocaleString() }}</TableCell>
              <TableCell class="text-right">
              ...
              <TableCell class="text-right">
                <Badge v-if="table.cache_hit_ratio !== null" :variant="getCacheHitVariant(table.cache_hit_ratio)">
        ...
        <Empty v-if="filteredTableIOStats.length === 0">
          <p class="text-muted-foreground">{{ $t('pages.database.io.no_table_stats') }}</p>
        </Empty>
      </Card>
        </div>
      </TabsContent>
      ...
      <TabsContent value="index" class="space-y-6">
        <!-- Index I/O Stats -->
        <div>
          <h3 class="text-lg font-semibold mb-3">{{ $t('pages.database.io.index_io_stats') }}</h3>
          <Card>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            <TableRow v-for="index in filteredIndexIOStats.slice(0, 30)" :key="`${index.schemaname}.${index.indexname}`">
              ...
              <TableCell class="text-right">{{ index.idx_blks_hit.toLocaleString() }}</TableCell>
              <TableCell class="text-right">
              ...
              <TableCell class="text-right">
                <Badge :variant="getCacheHitVariant(calculateCacheHitRatio(index))">
        ...
        <Empty v-if="filteredIndexIOStats.length === 0">
          <p class="text-muted-foreground">{{ $t('pages.database.io.no_index_io_stats') }}</p>
        </Empty>
      </Card>
        </div>
      </TabsContent>
...
export default {
  ...
  computed: {
    filteredTableIOStats() {
      ...
      return this.tableIOStats.filter((stat: any) => schemas.includes(stat.schemaname));
    },
    filteredIndexIOStats() {
      const schemas = this.selectedSchemas();
      if (schemas.length === 0) return this.indexIOStats;
      return this.indexIOStats.filter((stat: any) => schemas.includes(stat.schemaname));
    },
  },
  methods: {
